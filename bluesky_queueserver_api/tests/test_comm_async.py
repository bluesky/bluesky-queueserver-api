"""Tests for asynchronous communication classes"""
...
def test_ReManagerComm_ZMQ_Async_01():
    """
    ReManagerComm_ZMQ_Async: basic test.
...
def test_ReManagerComm_ZMQ_Async_02(monkeypatch, re_manager_cmd):  # noqa: F811
    """
    ReManagerComm_ZMQ_Async: Test if the setting
...
# fmt: on
def test_ReManagerComm_ZMQ_Async_03(monkeypatch, re_manager_cmd, zmq_encoding):  # noqa: F811
    """
    ReManagerComm_ZMQ_Async: Test if the setting ``zmq_encoding`` works as expected.
    """
    params = {"item": _plan1, "user": _user, "user_group": _user_group}
    ...
    async def testing():
        RM = ReManagerComm_ZMQ_Async(zmq_encoding=zmq_encoding, zmq_public_key=public_key)
        result = await RM.send_request(method="queue_item_add", params=params)
        assert result["success"] is True
        await RM.close()
...
def test_ReManagerComm_HTTP_Async_01():
    """
    ReManagerComm_HTTP_Async: basic test.
...
# fmt: on
def test_ReManagerComm_HTTP_Async_02(re_manager, fastapi_server, server_uri, success):  # noqa: F811
    """
    ReManagerComm_HTTP_Async: Test if parameter for setting HTTP server URI works as expected
    """
    params = {"item": _plan1}
...
def test_ReManagerComm_HTTP_Async_03():
    """
    ReManagerComm_HTTP_Async: Attempt to call unknown method, which does not exist
...
def test_ReManagerComm_ZMQ_Async_requests(re_manager, fastapi_server):  # noqa: F811
    """
    ReManagerComm_ZMQ_Async: Send a request: successful (accepted by the server),
    ...
    async def testing():
        ...
        RM = ReManagerComm_ZMQ_Async(request_fail_exceptions=False)
...
def test_ReManagerComm_HTTP_Async_requests(re_manager, fastapi_server):  # noqa: F811
    """
    ReManagerComm_HTTP_Async: Send a request: successful (accepted by the server),
    rejected and raising an exception, rejected not raising an exception.
    """
    params = {"item": _plan1}
    ...
    async def testing():
        ...
        # Do not raise the exceptions if the request fails (rejected by the server)
        RM = ReManagerComm_HTTP_Async(request_fail_exceptions=False)
        RM.set_authorization_key(api_key=API_KEY_FOR_TESTS)
        result = await RM.send_request(method="queue_item_add", params=params_invalid)
        assert result["success"] is False
        assert "request contains no item info" in result["msg"]
