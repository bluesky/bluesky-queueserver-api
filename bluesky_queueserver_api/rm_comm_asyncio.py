...
class ReManagerComm_Asyncio_HTTP(ReManagerAPI_Base):
    CommTimeoutError = CommTimeoutError
    ...
    def __init__(
        ...
        *,
        ...
        self._client = httpx.AsyncClient(base_url=http_server_uri, timeout=timeout / 1000)
    ...
    async def send_request(self, *, method, params=None):
        ...
        try:
            ...
            if self._timeout_exceptions:
                raise ReManagerComm_Asyncio_HTTP.RequestTimeoutError(
            ...
            if client_response.status_code < 500:
                ...
                raise ReManagerComm_Asyncio_HTTP.ClientError(message, exc.request, exc.response) from exc
            else:
                raise ReManagerComm_Asyncio_HTTP.ClientError(exc) from exc

        if self._request_failed_exceptions:
            # If the response is mapping, but it does not have 'success' field,
            #   then consider the request successful (this only happens for 'status' requests).
            if not isinstance(response, Mapping) or not response.get("success", True):
                raise ReManagerComm_Asyncio_HTTP.RequestFailedError(response)
